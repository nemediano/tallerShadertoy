...
\begin{itemize}
    ...
    \end{itemize}
    \item Las multiplicaciones: \say{matriz por vector}, \say{escalar por vector} y  \say{escalar por matriz} son las definida en álgebra lineal.
\end{itemize}
...
\begin{table}[htb]
  \begin{center}
    \begin{tabular}{l | l }
      ...
      \mintinline{glsl}{vec3 smoothstep(float e0, float e1, vec3 x)} & Interpolación de Hermite \\
      ...
      \mintinline{glsl}{float distance(vec3 x, vec3 y)} & Distancia entre dos vectores \\
    \end{tabular}
  \end{center}
\end{table}
